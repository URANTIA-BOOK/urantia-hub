# Versioned hub image for the lab conductor and self-host.
# NEXT_PUBLIC_* bake in at build time — pass them as build args.
...
COPY . .
ARG NEXT_PUBLIC_URANTIA_DEV_API_HOST=http://localhost:8080/dev-api
ARG NEXT_PUBLIC_HOST=http://localhost:8080
