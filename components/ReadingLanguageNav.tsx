...
const ReadingLanguageNav = ({
  ...
  const { language, source, availableLangs, setLanguage } = useReadingLanguage();
  const langs = availableLangs.length ? availableLangs : [ENGLISH_LANG];
  ...
  const navigatePaper = (nextLanguage: string, nextSource?: string | null) => {
    if (!paperId) return;
    void router.replace(
      paperPath(paperId, undefined, nextLanguage, nextSource),
      undefined,
      { shallow: true }
    );
  };
