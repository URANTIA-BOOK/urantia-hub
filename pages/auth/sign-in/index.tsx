...
const SignInPage = ({ email: emailEnabled, google }: SignInMethods) => {
  // Get the router.
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-100 via-white to-purple-100 text-gray-600 dark:text-white relative">
      ...
      <main className="flex flex-col items-center justify-center min-h-screen md:min-h-0 w-full max-w-md p-6 rounded bg-white shadow-lg">
...
export const getServerSideProps: GetServerSideProps<SignInMethods> = async () => {
  if (!isAuthEnabled()) {
    return {
      redirect: {
  ...
  return { props: configuredSignInMethods() };
};
